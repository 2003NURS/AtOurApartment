...
def register(request):
    if request.method == "POST":
        ...
        send_message = request.FILES.get("send_message")
...
def create_post(request):
    if request.method == 'POST':
        ...
        try:
            post = Post.objects.create(creater=request.user, content_text=text, content_image=pic, content_video= video)
            return HttpResponseRedirect(reverse('index'))
...
class EmailAttachementView(View):
    ...
    template_name = 'network/send_message.html'
    # template_name = 'network/login.html'

    def get(self, request):
        form = self.form_class()
        return render(request, 'network/send_message.html', {'email_form': form})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                mail.send()
                return render(request, 'network/send_message.html',
                              {'email_form': form,
...
class EmailAttachementView2(View):
    ...
    def get(self, request):
        form = self.form_class()
        return render(request, 'network/register.html', {'logIn': form})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                mail = EmailMessage(subject, settings.EMAIL_HOST_USER, [email])
                for f in files:
                    mail.attach(f.name, f.read(), f.content_type)
                mail.send()
...
def contact(request, contactUser=None):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "yes"
    return render(request, 'network/send_message.html', locals())

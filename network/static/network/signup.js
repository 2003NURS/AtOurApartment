document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".inp").forEach(input => {
        input.addEventListener('input', () => {
            ...
            if (input.classList.contains('usrname')) {
                if (input.value.trim().length !== 0) { check[0] = true; } else { check[0] = false; }
            }
            if (input.classList.contains('fname')) {
                if (input.value.trim().length !== 0) { check[1] = true; } else { check[1] = false; }
            }
            if (input.classList.contains('lname')) {
                if (input.value.trim().length !== 0) { check[2] = true; } else { check[2] = false; }
            }
            if (input.classList.contains('pswd')) {
                ...
                check[4] = false;
                if (input.value.trim().length !== 0) { check[3] = true; } else { check[3] = false; }
            }
            ...
            if (input.classList.contains('email')) {
                if (input.value.trim().length !== 0) { check[5] = true; } else { check[5] = false; }
            }
    ...
    //     Email.send({
    //             Host: "smtp.gmail.com",
    //             Username: "nursamanzhol@gmail.com",
    //             Password: "031028ser",
    //             To: 'nursamanzhol@gmail.com',
    //             From: document.getElementById("email").value,
    //             Subject: "This is new client",
    //             Body: "New client"
    //         })
